...
- DP (학습)
```bash
python finetune/dp.py \
## 추론 예시 (attention map 반환)

- NER
```bash
python inference/ner.py \
  --ckpt_dir ./outputs/ner \
  --tokens 나는 사과를 먹었다
```

- RE
```bash
python inference/re.py \
  --ckpt_dir ./outputs/re \
  --sentence "김철수는 삼성에 입사했다" \
  --subject '{"word":"김철수","type":"PER"}' \
  --object '{"word":"삼성","type":"ORG"}'
```

- DP
```bash
python inference/dp.py \
  --ckpt_dir ./outputs/dp \
  --tokens 나는 사과를 먹었다
```
